/*
 * Modified from pr2_pick_and_place_tutorial
 */
...
int main(int argc, char **argv)
{
  ...
  if(!pickup_client.waitForServer(ros::Duration(2.0)))
    exit(0);
  /*
  ...
  */
  ...
  processing_call.request.desired_frame = "base_link";// "wam_base_kinect_rgb_optical_frame";
  ...
  // Adjust size of graspable_object in planning scene / Remove collision objects touching top of graspable object
//  pickup_goal.target.potential_models.model_id = 
//  pickup_goal.target.potential_models.clear(); 
//  ROS_INFO_STREAM("potential models: " << pickup_goal.target.potential_models.size());
  ...
  direction.header.frame_id = "base_link";// "wam_base_kinect_rgb_optical_frame";
  direction.vector.x = 0;
  direction.vector.y = 0;
  direction.vector.z = 1;
  pickup_goal.lift.direction = direction;
  //request a vertical lift of 5cm after grasping the object
  ...
  // set loose path constraints
  pickup_goal.path_constraints.joint_constraints.resize(7);
  for (unsigned int i = 0; i <
       pickup_goal.path_constraints.joint_constraints.size(); ++i)
  {
    pickup_goal.path_constraints.joint_constraints.at(i).joint_name = "j" +
        boost::lexical_cast<std::string>(i+1);
    pickup_goal.path_constraints.joint_constraints.at(i).tolerance_above = 0.1;
    pickup_goal.path_constraints.joint_constraints.at(i).tolerance_below = 0.1;
  }
  //send the goal
  ...
  place_goal.desired_retreat_distance = 0.05;
  place_goal.min_retreat_distance = 0.025;
  ...
  //request a vertical put down motion of 5cm before placing the object 
  place_goal.approach.desired_distance = 0.05;
  place_goal.approach.min_distance = 0.025;
  //we are not using tactile based placing
  place_goal.use_reactive_place = false;
  // set loose path constraints
  place_goal.path_constraints.joint_constraints.resize(7);
  for (unsigned int i = 0; i <
       place_goal.path_constraints.joint_constraints.size(); ++i)
  {
    place_goal.path_constraints.joint_constraints.at(i).joint_name = "j" +
        boost::lexical_cast<std::string>(i+1);
    place_goal.path_constraints.joint_constraints.at(i).tolerance_above = 0.1;
    place_goal.path_constraints.joint_constraints.at(i).tolerance_below = 0.1;
  }
  //send the goal
